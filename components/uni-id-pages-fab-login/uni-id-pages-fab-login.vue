        ...
        servicesList: [
          {
            id: 'username',
            text: '账号登录',
            logo: this.$getOssFileByPath('/static/uni-fab-login/user.png'),
            path: '/uni_modules/uni-id-pages/pages/login/login-withpwd',
          },
          {
            id: 'smsCode',
            text: '验证码登录',
            logo: this.$getOssFileByPath('/static/uni-fab-login/sms.png'),
            path: '/uni_modules/uni-id-pages/pages/login/login-withoutpwd?type=smsCode',
          },
          {
            id: 'weixin',
            text: '微信登录',
            logo: this.$getOssFileByPath('/static/uni-fab-login/weixin.png'),
          },
          {
            id: 'apple',
            text: '苹果登录',
            logo: this.$getOssFileByPath('/static/uni-fab-login/apple.png'),
          },
          {
            id: 'univerify',
            text: '一键登录',
            logo: this.$getOssFileByPath('/static/uni-fab-login/univerify.png'),
          },
          {
            id: 'taobao',
            text: '淘宝登录', //暂未提供该登录方式的接口示例
            logo: this.$getOssFileByPath('/static/uni-fab-login/taobao.png'),
          },
          {
            id: 'facebook',
            text: '脸书登录', //暂未提供该登录方式的接口示例
            logo: this.$getOssFileByPath('/static/uni-fab-login/facebook.png'),
          },
          {
            id: 'alipay',
            text: '支付宝登录', //暂未提供该登录方式的接口示例
            logo: this.$getOssFileByPath('/static/uni-fab-login/alipay.png'),
          },
          {
            id: 'qq',
            text: 'QQ登录', //暂未提供该登录方式的接口示例
            logo: this.$getOssFileByPath('/static/uni-fab-login/qq.png'),
          },
          {
            id: 'google',
            text: '谷歌登录', //暂未提供该登录方式的接口示例
            logo: this.$getOssFileByPath('/static/uni-fab-login/google.png'),
          },
          {
            id: 'douyin',
            text: '抖音登录', //暂未提供该登录方式的接口示例
            logo: this.$getOssFileByPath('/static/uni-fab-login/douyin.png'),
          },
          {
            id: 'sinaweibo',
            text: '新浪微博', //暂未提供该登录方式的接口示例
            logo: this.$getOssFileByPath('/static/uni-fab-login/sinaweibo.png'),
          },
        ],
        univerifyStyle: {
          //一键登录弹出窗的样式配置参数
    ...
    methods: {
      ...
      navigateTo(path) {
        ...
        if (this.getRoute(1) == path.split('?')[0] && this.getRoute(1) == '/uni_modules/uni-id-pages/pages/login/login-withoutpwd') {
          //如果要被打开的页面已经打开，且这个页面是 /uni_modules/uni-id-pages/index/index 则把类型参数传给他
