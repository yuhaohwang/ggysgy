...
export default {
      ...
      servicesList: [
        {
          ...
          path: '/pages/uni-id-pages/login/login-withpwd',
        },
        {
          id: 'smsCode',
          text: '验证码登录',
          logo: '/static/uni-fab-login/sms.png',
          path: '/pages/uni-id-pages/login/login-withoutpwd?type=smsCode',
        },
        {
          id: 'weixin',
          text: '微信登录',
  ...
  methods: {
    ...
    navigateTo(path) {
      ...
      if (this.getRoute(1) == path.split('?')[0] && this.getRoute(1) == '/pages/uni-id-pages/login/login-withoutpwd') {
        //如果要被打开的页面已经打开，且这个页面是 /pages/uni-id-pages/index/index 则把类型参数传给他
    ...
    login(params, type) {
      ...
      let action =
        ...
        type
          .trim()
          .toLowerCase()
          .replace(type[0], type[0].toUpperCase())
