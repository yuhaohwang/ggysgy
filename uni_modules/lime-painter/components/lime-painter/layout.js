  ...
  getComputedStyle(element, parent = {}, index = 0) {
    ...
    for (let value of Object.keys(node)) {
      ...
      if (/padding|margin|radius/i.test(value)) {
        ...
        let pre = [0, 0, 0, 0].map((item, i) =>
          isRadius
            ...
            : [prefix + 'Top', prefix + 'Right', prefix + 'Bottom', prefix + 'Left'][i]
        )
        if (value === 'padding' || value === 'margin' || /^(border)?radius$/i.test(value)) {
          let v = item
            ?.split(' ')
            .map((item) => /^\d/.test(item) && toPx(item, node['width']), []) || [0]
      ...
      if (/^transform$/i.test(value)) {
        ...
        item.replace(/([a-zA-Z]+)\(([0-9,-\.%rpxdeg\s]+)\)/g, (g1, g2, g3) => {
          ...
          if (g2.includes('matrix')) {
            ...
            style[value][g2] = /[X]/.test(g2)
              ? transform(v[0], node['width'])
              : transform(v[0], node['height'])
          } else {
      ...
      if (/%/.test(item)) {
        ...
        style[value] = /px|rpx$/.test(item)
          ? toPx(item)
          : /em$/.test(item) && name == 'text'
          ? toPx(item, node['fontSize'])
          : item
      }
    }
    ...
    if (name === 'text') {
      ...
      if (!isOnly && !islineBlock) {
        ...
        lineIndex = Math.max(
          texts.length,
          Math.ceil(
            ctx.measureText(text, fontSize).width /
              ((w || pbox.width) - ctx.measureText('!', fontSize).width / 2)
          )
        )
      }
      if (!islineBlock) {
      ...
      if (!islineBlock) {
        box.offsetRight =
          (x || 0) + box.offsetLeft + (w ? w : isblock ? pbox.width : remain) + pr + mr
      }
      ...
      const _getWidth = () => {
        ...
        return (
          ...
          (!isOnly || isblock
            ? pbox.width
            : width > pbox.width - box.left || lineIndex > 1
            ? pbox.width - box.left
            : width)
        )
      }
      ...
      const _getTop = () => {
        ...
        return (
          _y + mt + ((isblock && ls?.lineHeight) || 0) + (lboxOffset + width > pbox.width ? lmb : 0)
        )
      }
      box.left = _getLeft()
      box.width = _getWidth()
      ...
      if (isNumber(x)) {
        ...
        box.left =
          ((lbox && (limageOffset < pbox.width ? limageOffset : pbox.left)) || pbox.left) + ml - mr
      }
      if (isNumber(w)) {
        box.width = w // - pl - pr
      } else {
      ...
      if (isNumber(y)) {
        ...
        box.top =
          ((lbox && (limageOffset < pbox.width ? limageOffset : lbox.top + lbox.height)) ||
            pbox.top) +
          mt +
          lmb
      }
  ...
  async getAttributes(element) {
    ...
    if (element?.url || element?.src) {
      ...
      const {
        ...
        path: src,
        originSrc,
      } = (await getImageInfo(arr.src, this.isH5PathToBase64)) || {}
