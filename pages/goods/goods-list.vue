<template>
	<view :class="!empty ? 'padding-top-big' : ''">
		<!-- 空白页 -->
		<use-empty v-if="empty" e-style="round" e-type="search" tip="搜索数据为空" btn-tip="重新搜索" height="70vh" :auto="false" @goto="tosearch"></use-empty>
		...
		<view v-else>
			...
			<view class="goods-list">
				<view class="list dflex-b dflex dflex-wrap-w w-full">
					<view class="item bg-main border-radius-sm padding-bottom-sm" v-for="(item, index) in goodsDatas" :key="index" @click="togoods(item)">
