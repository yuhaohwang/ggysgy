<template>
	<view class="fixed">
		<view class="y-s-c" style="width: 100vw; height: 100vh">
			<view class="flex1 y-s-c padding gradient-bg" style="overflow: hidden; border-radius: 40px 40px 0px 0px">
				<view class="fixed x-c-c" style="left: 50rpx">
					<image :src="$getOssFileByPath('/static/logo/logo.png')" class="border-radius-c headimg"
						style="width: 50rpx; height: 50rpx"></image>
					<view class="margin-left-xs">关注</view>
				</view>
				...
				<scroll-view class="flex1 y-c-c margin-top" scroll-y style="min-height: 0rpx;" @scroll="setTotop">
						...
						<swiper class="w-full" style="height: 600rpx" indicator-dots circular="true" duration="400">
							<swiper-item v-for="(item, index) in swiperDatas" :key="index">
								<view class="wh-full x-c-c">
									<image :src="item.url" class="swiper-img border-radius" style="height: 100%" lazy-load="true"
										mode="heightFix" @load="setContainerHeight"></image>
								</view>
							</swiper-item>
						</swiper>
					</view>
					...
					<view class="x-c-c margin-top-xs padding-lr-lg text-center">
				...
				<view>
					...
					<view class="x-c-c">
						...
						<view class="padding-xs">
							<image :src="$getOssFileByPath('/static/tabbar/user.png')" class="border-radius-c headimg"
								style="width: 50rpx; height: 50rpx"></image>
						</view>
					</view>
				</view>
			...
			<!-- 07. 操作区 -->
			<view class="oper-area w-full x-b-c padding-lr-lg">
		...
		<block v-if="posterShow">
			<l-painter custom-style="position: fixed; left: -100vw;" :board="posterData" isCanvasToTempFilePath
				@success="posterSuccess" @fail="fail = $event" @progress="log = $event" />
		</block>
		...
		<!-- 置顶 -->
		<use-totop ref="usetop" bottom="120" @toTop="test"></use-totop>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			createPoster() {
				...
				this.posterQRcode =
					`https://ggysgy-h5.use-cloud.com/#/pages/goods/goods?id=${this.goods._id}&mid=${this.member._id}`;
				// #endif
			},
			// 海报二维码生成成功
