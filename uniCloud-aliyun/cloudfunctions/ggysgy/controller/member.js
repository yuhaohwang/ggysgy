...
module.exports = class MemberController extends Controller {
  ...
  async register() {
    ...
    if (res.code == 0) {
      ...
      if (res.username.length == 11) {
        nickname = '艺心益盟-' + res.username.substr(7, 4);
      }
  ...
  async sendSmsCode() {
    ...
    const res = await uidObj.sendSmsCode({
      ...
      templateId: uidObj.pluginConfig.defaultConfig['service']['smscode']['templateId'],
    });
